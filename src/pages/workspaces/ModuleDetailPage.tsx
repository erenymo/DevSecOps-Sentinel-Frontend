...
import type {
  Module,
  Component,
  VulnerabilityAiInsight,
  PackageVulnerabilityInsight,
...
export function ModuleDetailPage() {
  ...
  const [depSort, setDepSort] = useState<{
    key: "name" | "license";
    direction: "asc" | "desc";
  ...
  const [vulnSort, setVulnSort] = useState<{
    key: "component" | "severity" | "status";
    direction: "asc" | "desc";
  } | null>({ key: "severity", direction: "desc" });
  const [expandedVulnGroups, setExpandedVulnGroups] = useState<Set<string>>(
    new Set(),
  );
  const [expandedLicenseRows, setExpandedLicenseRows] = useState<Set<string>>(
    new Set(),
  );
  ...
  const { data: componentsData, isLoading: isComponentsLoading } =
    useModuleComponents(moduleId || "");
  const components = componentsData?.data || [];

  const { data: scanStatusResult } = useScanStatus(moduleId || "");
  const scanStatus = scanStatusResult?.data;
  const isEnrichingLicenses = scanStatus?.isDependenciesParsed
    ? !scanStatus.isLicenseEnrichmentCompleted
    : false;
  const isEnrichingVulns = scanStatus?.isDependenciesParsed
    ? !scanStatus.isVulnEnrichmentCompleted
    : false;

  const { data: vulnAiInsightResult } = useVulnerabilityInsights(
    moduleId || "",
  );
    ...
    <div className="p-12 flex flex-col justify-center items-center gap-4 max-w-md mx-auto mt-4">
      ...
      <div className="space-y-1 text-center w-full mt-2">
        <p className="text-sm font-semibold text-foreground">
          Fetching Data / Analysis in Progress
        </p>
  ...
  const uniqueVulnsMapGlobal = new Map<
    string,
    (typeof rawVulnerabilities)[0]
  >();
  ...
  const totalPackagesToAnalyze = Math.min(
    100,
    new Set(allVulnerabilities.map((v) => v.componentName)).size,
  );
  const analyzedCount = vulnAiInsight?.packageInsights?.length || 0;
  const unanalyzedCount = Math.max(0, totalPackagesToAnalyze - analyzedCount);

  useEffect(() => {
    if (isAutoAnalyzing) {
      if (unanalyzedCount > 0 && !analyzeVulnerabilityInsights.isPending) {
        analyzeVulnerabilityInsights.mutate();
  ...
  const filteredVulnerabilities = allVulnerabilities.filter((v) => {
    const matchesSearch = v.componentName
      .toLowerCase()
      .includes(vulnSearchTerm.toLowerCase());
    const matchesSeverity =
      vulnSeverityFilter === "all" ||
      v.severityLevel?.toUpperCase() === vulnSeverityFilter.toUpperCase();
  ...
  groupedVulnerabilities.forEach((group) => {
    group.vulnerabilities.sort(
      (a, b) => (b.severityScore ?? 0) - (a.severityScore ?? 0),
    );
  });

  const topLevelComponents = components.filter((c) => !c.isTransitive);

  let filteredTopLevelComponents = topLevelComponents.filter((c) => {
    const directMatches = c.name
      .toLowerCase()
      .includes(depSearchTerm.toLowerCase());
    const transitiveMatches = components.some(
      (child) =>
        child.isTransitive &&
  ...
  if (depSort) {
    filteredTopLevelComponents.sort((a, b) => {
      ...
      if (depSort.key === "name") {
        ...
        valA =
          a.licenseNames && a.licenseNames[0]
            ? a.licenseNames[0].toLowerCase()
            : "z";
        valB =
          b.licenseNames && b.licenseNames[0]
            ? b.licenseNames[0].toLowerCase()
            : "z";
      }
  ...
  const renderComponentRow = (comp: Component, depth: number = 0) => {
    ...
    const isExpanded =
      expandedRows.has(comp.id) || (!!depSearchTerm && hasChildren);

    return (
      <Fragment key={`row-${comp.id}`}>
        <tr
          className={`transition-colors border-b ${comp.isTransitive ? "bg-muted/20 dark:bg-muted/5 text-muted-foreground hover:bg-muted/30" : "hover:bg-muted/50 font-medium"}`}
        >
          ...
          <td
            className={`px-4 py-3 ${comp.isTransitive ? "text-xs" : "text-sm"}`}
          >
            {comp.version}
          </td>
          <td className="px-4 py-3">
            <span
              className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${comp.isTransitive ? "bg-secondary/60 text-secondary-foreground" : "bg-primary/10 text-primary"}`}
            >
            ...
            <div className="flex flex-wrap gap-1">
              {comp.licenseNames && comp.licenseNames.length > 0 ? (
                comp.licenseNames.map((l, i) => (
                  <span
                    key={i}
                    className={`inline-flex items-center rounded-sm px-1.5 py-0.5 text-[9px] font-medium ${getLicenseColor(l)}`}
                  >
                    {l}
                  </span>
                ))
        ...
        {isExpanded &&
          children.map((child) => renderComponentRow(child, depth + 1))}
      </Fragment>
    );
  };
  ...
  const licenseIssuesCount = components.filter(
    (c) =>
  ...
  const tabs = [
    ...
    },
    {
      ...
      color: "text-red-500",
      bg: "bg-red-500/10",
      description: "Security vulnerabilities detected across dependencies.",
    },
    {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm line-clamp-1">
              {module?.ecosystem || ""}
              {module?.rootPath ? ` · ${module.rootPath}` : ""}
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6">
        {tabs.map((tab) => (
          <Card key={tab.label} className="border bg-card shadow-sm">
            ...
            <CardContent>
              ...
              {tab.label === "Dependencies" ? (
                      ...
                      <table className="w-full min-w-[600px] text-sm text-left border-collapse">
                        <thead className="sticky top-0 bg-muted/95 dark:bg-slate-900/95 backdrop-blur-sm text-muted-foreground z-10 shadow-[0_1px_0_0_rgba(0,0,0,0.05)]">
                          <tr>
                            <th className="px-4 py-3 font-medium">
                              <button
                                onClick={() => handleDepSort("name")}
                                className="flex items-center gap-1 hover:text-foreground transition-colors"
                              >
                                Name
                                <ArrowUpDown
                                  className={`w-3 h-3 ${depSort?.key === "name" ? "text-primary" : ""}`}
                                />
                              </button>
                            </th>
                            ...
                            <th className="px-4 py-3 font-medium">
                              <button
                                onClick={() => handleDepSort("license")}
                                className="flex items-center gap-1 hover:text-foreground transition-colors"
                              >
                                License
                                <ArrowUpDown
                                  className={`w-3 h-3 ${depSort?.key === "license" ? "text-primary" : ""}`}
                                />
                              </button>
                            </th>
                          </tr>
                        ...
                        <tbody className="divide-y">
                          {filteredTopLevelComponents.length > 0 ? (
                            filteredTopLevelComponents.map((c) =>
                    ...
                    {vulnAiInsight ? (
                      (() => {
                        const score = Math.min(
                          10,
                          Math.max(0, vulnAiInsight.systemCriticalityScore),
                        );
                        const radius = 28;
                        ...
                        return (
                          <Card
                            className={`bg-gradient-to-r ${gradientFrom} via-red-500/5 to-purple-500/10 border-red-500/20 shadow-none`}
                          >
                                ...
                                <div className="flex-1 space-y-3">
                                  <div>
                                    <h4 className="text-sm font-semibold text-foreground mb-1.5 flex items-center gap-2">
                                      AI Threat Analysis
                                      <span className="inline-flex items-center rounded-full bg-red-500/20 text-red-500 px-2 py-0.5 text-[10px] font-bold">
                                        CTO/CISO
                                      </span>
                                    </h4>
                                  ...
                                  <div className="border-t border-red-500/10 pt-2.5">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                      ...
                                      {(() => {
                                        ...
                                        const analyzedCount =
                                          vulnAiInsight?.packageInsights
                                            ?.length || 0;
                                        const unanalyzedCount = Math.max(
                                          0,
                                          totalPackagesToAnalyze -
                                            analyzedCount,
                                        );
                                        if (unanalyzedCount > 0) {
                                          return (
                                            <Button
                                              size="sm"
                                              className="h-6 text-[10px] bg-red-600/10 hover:bg-red-600/20 text-red-500 border border-red-500/20 shadow-none"
                                    ...
                                    <div className="mt-4 pt-4 border-t border-red-500/10">
                                      <h5 className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider mb-3">
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {(() => {
                                            ...
                                            const aiPkgInsight =
                                              vulnAiInsight?.packageInsights?.find(
                                                (p) =>
                                                  ...
                                                  pkg.componentName?.toLowerCase(),
                                              );
                                            let fixedVersion =
                                              aiPkgInsight?.fixedVersionRecommendation;
                                            if (!fixedVersion) {
                                              const vWithFix =
                                                pkg.vulnerabilities.find(
                                                  (v: any) => v.fixedVersion,
                                                );
                                              fixedVersion = vWithFix
                                                ? vWithFix.fixedVersion
                                                : "No fix available";
                                            }

                                            return (
                                              <div
                                                key={pkg.componentName}
                                                className="bg-background rounded-md border border-muted/60 p-2.5 shadow-sm flex items-center justify-between hover:border-red-500/40 transition-colors"
                                              >
                                                <div className="flex flex-col gap-1 min-w-0 w-full">
                                                  <div className="flex items-center justify-between gap-2">
                                                    <span
                                                      className="font-bold text-xs text-foreground truncate"
                                                      title={pkg.componentName}
                                                    >
                                                      {idx + 1}.{" "}
                                                      {pkg.componentName}
                                                    </span>
                                                    <span
                                                      className={`shrink-0 inline-flex items-center rounded-sm px-1.5 py-0.5 text-[9px] font-bold ${pkg.maxSeverityScore >= 7.0 ? "bg-red-500/10 text-red-600 border border-red-500/20" : pkg.maxSeverityScore >= 4.0 ? "bg-amber-500/10 text-amber-600 border border-amber-500/20" : "bg-green-500/10 text-green-600 border border-green-500/20"}`}
                                                    >
                                                      {pkg.maxSeverityLevel} (
                                                      {pkg.maxSeverityScore.toFixed(
                                                        1,
                                                      )}
                                                      )
                                                    </span>
                                          ...
                                          return (
                                            <>
                                              <div>
                                                <h6 className="text-[11px] font-bold uppercase tracking-wider text-indigo-500/80 mb-2 flex items-center gap-1.5">
                                                  <Layers className="w-3.5 h-3.5" />{" "}
                                                  Top 5 Components with the Most
                                                  Vulnerabilities
                                                </h6>
                                                <div className="space-y-2">
                                                  {topByCount.map((pkg, idx) =>
                                                    renderPriorityCard(
                                                      pkg,
                                                      idx,
                                                    ),
                                              ...
                                              <div>
                                                <h6 className="text-[11px] font-bold uppercase tracking-wider text-red-500/80 mb-2 flex items-center gap-1.5">
                                                  <ShieldAlert className="w-3.5 h-3.5" />{" "}
                                                  Top 5 Components with the
                                                  Highest Risks
                                                </h6>
                                                <div className="space-y-2">
                                                  {topBySeverity.map(
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Analyze all vulnerable packages to generate VEX
                              triage recommendations and secure alternatives.
                            </p>
                          </div>
                        </div>
                        <Button
                      ...
                      <table className="w-full min-w-[600px] text-sm text-left">
                        <thead className="bg-muted/50 text-muted-foreground">
                          <tr>
                            <th className="px-4 py-3 font-medium">
                              <button
                                onClick={() => handleVulnSort("component")}
                                className="flex items-center gap-1 hover:text-foreground transition-colors"
                              >
                                Component
                                <ArrowUpDown
                                  className={`w-3 h-3 ${vulnSort?.key === "component" ? "text-primary" : ""}`}
                                />
                              </button>
                            </th>
                            <th className="px-4 py-3 font-medium">Version</th>
                            ...
                            <th className="px-4 py-3 font-medium">
                              <button
                                onClick={() => handleVulnSort("severity")}
                                className="flex items-center gap-1 hover:text-foreground transition-colors"
                              >
                                Severity
                                <ArrowUpDown
                                  className={`w-3 h-3 ${vulnSort?.key === "severity" ? "text-primary" : ""}`}
                                />
                              </button>
                            </th>
                            <th className="px-4 py-3 font-medium">
                              ...
                              >
                                Status
                                <ArrowUpDown
                                  className={`w-3 h-3 ${vulnSort?.key === "status" ? "text-primary" : ""}`}
                                />
                              </button>
                            </th>
                        ...
                        <tbody className="divide-y">
                          {groupedVulnerabilities.map((group) => {
                            const isExpanded =
                              expandedVulnGroups.has(group.componentName) ||
                              !!vulnSearchTerm;
                            return (
                              <Fragment key={group.componentName}>
                                <tr
                                  onClick={(e) =>
                                    toggleVulnGroup(group.componentName, e)
                                  }
                                ...
                                >
                                  <td className="px-4 py-3 font-semibold flex items-center">
                                    <div className="flex items-center">
                                      ...
                                      <span className="text-sm font-semibold text-foreground flex items-center gap-2">
                                        {group.componentName}
                                        {vulnAiInsight?.packageInsights?.some(
                                          (pi) =>
                                            pi.packageName?.toLowerCase() ===
                                            group.componentName?.toLowerCase(),
                                        ) && (
                                  ...
                                  <td className="px-4 py-3">
                                    <span
                                      className={`inline-flex items-center rounded-sm px-1.5 py-0.5 text-[10px] font-semibold ${getSeverityColor(group.maxSeverityLevel)}`}
                                    >
                                      {group.maxSeverityLevel}{" "}
                                      {group.maxSeverityScore > 0
                                        ? `(${group.maxSeverityScore})`
                                        : ""}
                                    </span>
                                  </td>
                                  <td className="px-4 py-3 text-xs text-muted-foreground font-normal">
                                    Expand to manage VEX
                                  </td>
                                      ...
                                      <tr className="bg-muted/5 dark:bg-muted/2 border-b">
                                        <td colSpan={6} className="px-6 py-5">
                                          <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                            ...
                                            {pkgInsight.alternatives &&
                                              pkgInsight.alternatives.length >
                                                0 && (
                                                <div>
                                                  <h5 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
                                                    ...
                                                    Recommended MIT / Apache
                                                    Alternatives
                                                  </h5>
                                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {pkgInsight.alternatives.map(
                                                      (alt, i) => (
                                                        <Card
                                                          key={i}
                                                          className="bg-background shadow-sm border-dashed hover:border-green-500/30 transition-colors"
                                                        >
                                ...
                                {isExpanded &&
                                  group.vulnerabilities.map((v) => (
                                    <tr
                                      key={`${v.componentId}-${v.externalId}`}
                                      className="bg-muted/5 dark:bg-muted/2 hover:bg-muted/10 transition-colors border-b text-muted-foreground text-xs"
                                    >
                                      ...
                                      <td className="px-4 py-2.5">
                                        <span
                                          className={`inline-flex items-center rounded-sm px-1.5 py-0.5 text-[9px] font-medium ${getSeverityColor(v.severityLevel)}`}
                                        >
                                          {v.severityLevel || "UNKNOWN"}{" "}
                                          {v.severityScore !== undefined &&
                                          v.severityScore !== null
                                            ? `(${v.severityScore})`
                                            : ""}
                                        </span>
                                      </td>
                                      ...
                                      <td className="px-4 py-2.5 text-green-600 dark:text-green-500 font-medium">
                                        {v.fixedVersion || "-"}
                                      </td>
                                    </tr>
                                  ))}
                ...
                (() => {
                  const licenseIssueComponents = components.filter(
                    (c) =>
                      c.licenseNames &&
                      c.licenseNames.some((l) => getLicenseRisk(l) !== "low"),
                  );

                  let score = 100;
                  licenseIssueComponents.forEach((comp) => {
                    const isHighRisk = comp.licenseNames?.some(
                      (l) => getLicenseRisk(l) === "high",
                    );
                    const isMediumRisk = comp.licenseNames?.some(
                      (l) => getLicenseRisk(l) === "medium",
                    );
                          ...
                          <div className="w-full sm:w-[80%] p-5 flex items-start gap-4">
                            ...
                            <div>
                              <h4 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-2">
                                AI License Insights
                                <span className="inline-flex items-center rounded-full bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 text-[10px] font-bold">
                                  Beta
                                </span>
                              </h4>
                              <p className="text-xs text-muted-foreground leading-relaxed font-medium">
                              ...
                              <div className="mt-3 flex items-center gap-4 text-[11px] text-muted-foreground/80 font-medium">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  {
                                    licenseIssueComponents.filter((c) =>
                                      c.licenseNames?.some(
                                        (l) => getLicenseRisk(l) === "high",
                                      ),
                                    ).length
                        ...
                        <table className="w-full min-w-[600px] text-sm text-left">
                          ...
                          <tbody className="divide-y">
                            {licenseIssueComponents.map((comp) => {
                              const highRiskLicenses =
                                comp.licenseNames?.filter(
                                  (l) => getLicenseRisk(l) === "high",
                                ) || [];
                              ...
                              return (
                                <Fragment key={comp.id}>
                                  <tr
                                    onClick={(e) =>
                                      comp.aiInsight &&
                                      toggleLicenseRow(comp.id, e)
                                    }
                                    className={`transition-colors border-b ${comp.aiInsight ? "cursor-pointer hover:bg-muted/50" : "hover:bg-muted/30"}`}
                                  >
                                    <td className="px-4 py-3 text-center w-12 align-middle">
                                      {comp.aiInsight ? (
                                        ...
                                        <span className="text-[9px] leading-tight font-semibold text-muted-foreground/60 select-none block text-center">
                                          Analysis
                                          <br />
                                          Pending
                                        </span>
                                    ...
                                    <td className="px-4 py-3">
                                      <span
                                        className={`inline-flex items-center rounded-sm px-2 py-0.5 text-[10px] font-bold border ${
                                          isHighRisk
                                            ? "bg-red-500/10 text-red-500 border-red-500/20"
                                    ...
                                    <td className="px-4 py-3 text-xs">
                                      {isHighRisk ? (
                                        <span className="text-red-600 dark:text-red-400 font-medium">
                                          Copyleft - Commercial distribution
                                          requires open-sourcing derived works.
                                        </span>
                                      ) : (
                                        <span className="text-amber-600 dark:text-amber-400 font-medium">
                                          Weak Copyleft - Modifications to this
                                          library itself must be open-sourced.
                                        </span>
                                      )}
                                    </td>
                                  </tr>
                                          ...
                                          <div className="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                                            <div className="bg-background rounded-lg border p-4 shadow-sm">
                                              ...
                                              <p className="text-sm text-foreground/90 leading-relaxed font-medium mb-4">
                                                {
                                                  comp.aiInsight
                                                    .riskExplanationForManagement
                                                }
                                              </p>

                                              {/* Two-Column Conditional Risk Layout */}
                                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                                <div className="bg-red-500/5 border border-red-500/20 rounded-md p-3">
                                                  <h6 className="text-[11px] font-bold uppercase tracking-wider text-red-600 dark:text-red-400 mb-2 flex items-center gap-1.5">
                                                ...
                                                <div className="bg-green-500/5 border border-green-500/20 rounded-md p-3">
                                                  <h6 className="text-[11px] font-bold uppercase tracking-wider text-green-600 dark:text-green-400 mb-2 flex items-center gap-1.5">
                                                ...
                                                {(() => {
                                                  try {
                                                    const alts = JSON.parse(
                                                      comp.aiInsight
                                                        .recommendedAlternativesJson,
                                                    );
                                                    return alts.map(
                                                      (alt: any, i: number) => (
                                                        <Card
                                                          key={i}
                                                          className="bg-background shadow-sm border-dashed"
                                                        >
                                                          <CardContent className="p-4 flex flex-col gap-2">
                                                            <div className="flex justify-between items-start">
                                                              <span className="font-bold text-sm text-primary">
                    ...
                    <div className="p-8 border border-dashed rounded-lg bg-green-500/5 dark:bg-green-500/2 text-center text-green-600 dark:text-green-400 text-sm font-semibold flex flex-col items-center justify-center gap-3 animate-in fade-in zoom-in duration-300">
                      ...
                      <div>
                        <h4 className="text-base font-semibold text-foreground mb-1">
                          Compliance Shield Clear
                        </h4>
                        <p className="text-xs text-muted-foreground max-w-md font-normal mx-auto">
                          No copyleft or restrictive licenses detected. All
                          dependency licenses comply with low-risk commercial
                          policies.
                        </p>
                      </div>
                    </div>
